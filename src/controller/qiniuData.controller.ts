...
class QiniuController {
  ...
  deleteByQiniuKey = async (ctx: ParameterizedContext, next) => {
  ...
  getDiff = async (ctx: ParameterizedContext, next) => {
    ...
    Object.keys(qiniuOfficialResMap).forEach((item) => {
      if (qiniuOfficialResMap[item] && !qiniuDataResMap[item]) {
        officialDiff.push(item);
      }
    });
    // 遍历qiniudata
    Object.keys(qiniuDataResMap).forEach((item) => {
      if (qiniuDataResMap[item] && !qiniuOfficialResMap[item]) {
