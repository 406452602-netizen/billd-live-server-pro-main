...
import { jwtVerify, signJwt } from '@/app/auth/authJwt';
...
class WechatUserController {
  ...
  login = async (ctx: ParameterizedContext, next) => {
    ...
    const { code, platform }: { code: string; platform: string } = loginBody;
    ...
    if (accessToken.errcode) {
      throw new CustomError({
        msg: JSON.stringify(accessToken),
        httpStatusCode: COMMON_HTTP_CODE.paramsError,
        errorCode: COMMON_HTTP_CODE.paramsError,
      });
    }
    ...
    if (!isExist) {
      ...
      if (!thirdUserInfo) {
        throw new CustomError({
          msg: `wechat登录thirdUserInfo错误`,
          httpStatusCode: COMMON_HTTP_CODE.paramsError,
          errorCode: COMMON_HTTP_CODE.paramsError,
        });
      }
      const userRes = await userService.find(thirdUserInfo.user_id!);
      if (!userRes) {
        throw new CustomError({
          msg: `wechat登录userRes错误`,
          httpStatusCode: COMMON_HTTP_CODE.paramsError,
          errorCode: COMMON_HTTP_CODE.paramsError,
        });
      }
