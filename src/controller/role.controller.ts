...
class RoleController {
  ...
  // 获取所有角色（树型）
  getTreeRole = async (ctx: ParameterizedContext, next) => {
    const {
      type,
      priority,
      keyWord,
  ...
  // 获取某个角色的权限
  getRoleAuth = async (ctx: ParameterizedContext, next) => {
    const id = +ctx.params.id;
  ...
  // 获取某个角色的权限（递归找所有）
  getAllRoleAuth = async (ctx: ParameterizedContext, next) => {
    const id = +ctx.params.id;
    const result = await roleService.getRoleAuth(id);
    successHandler({ ctx, data: { total: result.length, result } });
    await next();
  ...
  // 获取某个用户的角色
  getUserRole = async (ctx: ParameterizedContext, next) => {
  ...
  // 修改某个角色的权限
  updateRoleAuth = async (ctx: ParameterizedContext, next) => {
    const id = +ctx.params.id;
    ...
    if (!isExistAuth) {
      throw new CustomError({
        msg: `${role_auths.toString()}中存在不存在的权限！`,
        httpStatusCode: COMMON_HTTP_CODE.paramsError,
        errorCode: COMMON_HTTP_CODE.paramsError,
      });
    }
  ...
  update = async (ctx: ParameterizedContext, next) => {
    ...
    if (id === 1) {
      ...
      if (!isExist) {
        throw new CustomError({
          msg: `${[id, p_id].toString()}中存在不存在的角色！`,
          httpStatusCode: COMMON_HTTP_CODE.paramsError,
          errorCode: COMMON_HTTP_CODE.paramsError,
        });
      }
  ...
  batchDeleteChildRoles = async (ctx: ParameterizedContext, next) => {
    ...
    if (!isExist) {
      throw new CustomError({
        msg: `${[id, ...c_roles].toString()}中存在不存在的角色！`,
        httpStatusCode: COMMON_HTTP_CODE.paramsError,
        errorCode: COMMON_HTTP_CODE.paramsError,
      });
    }
    ...
    if (hasDiff.length) {
      throw new CustomError({
        msg: `${c_roles.toString()}中的角色父级id不是${id}！`,
        httpStatusCode: COMMON_HTTP_CODE.paramsError,
        errorCode: COMMON_HTTP_CODE.paramsError,
      });
    }
  ...
  batchAddChildRoles = async (ctx: ParameterizedContext, next) => {
    ...
    if (!isUnique) {
      throw new CustomError({
        msg: `${c_roles.toString()}不是同一个父级角色！`,
        httpStatusCode: COMMON_HTTP_CODE.paramsError,
        errorCode: COMMON_HTTP_CODE.paramsError,
      });
    }
