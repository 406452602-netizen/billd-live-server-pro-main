...
class InitController {
  common = {
    initDefault: async () => {
      try {
        await Promise.all([
          ...
          this.common.initDayData(365 * 1),
          this.common.initHourData(365 * 1 * 24),
          this.common.initMinuteTenData(365 * 1 * 24 * 6),
          this.common.initMinuteThirtyData(365 * 1 * 24 * 2),
    ...
    initArea: async () => {
      ...
      if (count === 0) {
        const flatdata: any[] = [];
        bulkCreateArea().forEach((item) => {
          flatdata.push(...flattenTree(item, 'children'));
        });
        await areaModel.bulkCreate(flatdata);
      } else {
        throw new CustomError({
          msg: '已经初始化过分区，不能再初始化了！',
    ...
    initDayData: async (total = 365 * 3) => {
      ...
      // } else {
      //   throw new CustomError({
      //     msg: `已经初始化过${mockDayDataModel.tableName}表了，不能再初始化了！`,
      //     httpStatusCode: COMMON_HTTP_CODE.paramsError,
      //     errorCode: COMMON_HTTP_CODE.paramsError,
      //   });
      // }
  ...
  // 更新所有直播间的推拉流地址
  updateAllLiveRoomUrl = async (ctx: ParameterizedContext, next) => {
    const res = await liveRoomController.common.findAll();
    const queue: any[] = [];
    res.forEach((item) => {
      const fn = async () => {
        const userLiveRoom =
          await userLiveRoomController.common.findByLiveRoomId(item.id!);
        if (userLiveRoom) {
    ...
    successHandler({ ctx, msg: '更新所有直播间的推拉流地址成功！' });
    await next();
  };
  ...
  // 初始化分区
  initArea = async (ctx: ParameterizedContext, next) => {
  ...
  deleteUser = async (ctx: ParameterizedContext, next) => {
    ...
    if (!res1.count) {
      throw new CustomError({
        msg: `不存在id为${userId as string}的用户！`,
        httpStatusCode: COMMON_HTTP_CODE.paramsError,
        errorCode: COMMON_HTTP_CODE.paramsError,
      });
    }
