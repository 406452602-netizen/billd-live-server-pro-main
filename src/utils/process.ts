...
export function pushToBilibili(flag = true) {
  ...
  const cmd = `ffmpeg -threads 1 -readrate 1 -stream_loop -1 -f concat -safe 0 -i '${listFile}' -vcodec h264 -acodec aac -f flv xxxxx`;
  try {
    if (flag) {
      exec(cmd);
    ...
    console.error(chalkERROR(`FFmpeg推流到bilibili失败`));
    console.log(error);
  }
}
...
export function forwardThirdPartyLiveStreaming({
  ...
  try {
    ...
    console.error(chalkERROR(`FFmpeg转推到${platform}失败`));
    console.log(error);
  }
}
