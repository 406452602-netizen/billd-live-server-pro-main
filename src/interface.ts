...
/**
 * 带cols属性的分页接口，扩展IPaging
 */
export interface IPagingWithCols<T, C = any> extends IPaging<T> {
  cols: C;
}
...
export interface IInviteAgent {
  ...
  agent_account_for?: number;
  is_valid?: boolean;
  expiration_time?: string;
  be_invite_user_id?: number;
...
export interface IGameConsumptionRecord {
  ...
  has_processed?: boolean; // 标记是否已处理，用于VBOSS结算避免重复处理
  created_at?: string;
  updated_at?: string;
  deleted_at?: string;
}
