...
class UserLiveRoomService {
  ...
  async findByUserIdList(user_id: number) {
    const result = await userLiveRoomModel.findAll({
      include: [
        {
          model: userModel,
  ...
  async create({ user_id, live_room_id }: IUserLiveRoom) {
    const result = await userLiveRoomModel.create({ user_id, live_room_id });
    return result;
  }
  ...
  async delete(id: number) {
    const result = await userLiveRoomModel.destroy({ where: { id }, limit: 1 });
    return result;
  }
}
