...
export function getSocketRealIp(socket: Socket) {
  const realIp = strSlice(
    String(socket?.handshake?.headers?.['x-real-ip']),
    90
  );
  return realIp;
}
...
export function getSocketUserAgent(socket: Socket) {
  const ua = strSlice(String(socket?.handshake?.headers?.['user-agent']), 490);
  return ua;
}
